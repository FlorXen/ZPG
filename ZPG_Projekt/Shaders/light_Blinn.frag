...
void main() {
    ...
    for (int index = 0; index < numberOfLights; index++) 
    {
        ...
        float specularFactor = pow(max(dot(halfVector, worldNormal), 0.0), material.shininess);
        ...
        switch (lights[index].lightType) {
            ...
            case 2: {
                ...
                halfVector = normalize(lightVector + camVector);
                dot_product = max(dot(worldNormal, lightVector), 0.0);
                diffuse = dot_product * lights[index].diffuse * material.diffuse;

                if (dot(lightVector, worldNormal) > 0.0) {
                    specularFactor = pow(max(dot(worldNormal, halfVector), 0.0), material.shininess);
